...
function stockDelete(stockNo, productNo, count){
	if(confirm("Do you want to Delete?")){
		location.href = "stockDelete?stockNo=" + stockNo
									+"&productNo=" + productNo
									+"&count=" + count;
	}
}
		...
		<div class="right">
			...
			<div class="content">
				...
				<table style="table-layout: fixed; width: 80%;">
					<tr><th class="th1">no.</th><th>name</th><th>comments</th><th>입고일</th><th>제품</th><th>수량</th><th>거래처</th><th>검수</th><th>modify</th><th>delete</th></tr>
					<c:if test="${stock.size() eq 0 or stock eq null }">
						<tr>
						...
						<tr>
							...
							<td>
								<input type="button" style="width:50px; cursor: pointer;" value="수정" 
									onclick="location.href='stockInfo?stockNo=${entry.stockNo }'">
							</td>
							<td>
								<c:choose>
									...
									<c:otherwise>
										<input type="button" style="width:50px; cursor: pointer;" value="삭제" 
											onclick="stockDelete('${entry.stockNo }','${entry.productNo }','${entry.count }')">	
									</c:otherwise>
								</c:choose>
							</td>
						</tr>
					...
					<tr>
						<td colspan="10">
							<div align="center">
								<c:if test="${current > 1 }">
									<a href="stockList?num=${current -1}">[이전]</a>
								</c:if>
	                  			<c:forEach var="num" begin="1" end="${repeat }">
		                     		<a href="stockList?num=${num }">[${num }]</a>
	                  			</c:forEach>
	                  			<c:if test="${current ne repeat && current > 1}">
									<a href="stockList?num=${current +1}">[다음]</a>
								</c:if>
               				</div>
						</td>
					</tr>
					<tr>
						<td colspan="10">
							<div align="center">
								<input type="button" style="width:200px; cursor: pointer;" value="입고 등록" onclick="location.href='stockForm'">
							</div>
						</td>
					</tr>
				</table>
